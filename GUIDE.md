...
3. **Baisse `min_profit_threshold`** (ex. `0.002`) pour voir les écarts plus
   petits.
4. **Choisis le mode « Automatique »** au moment des paires dans `./install.sh` :
   le bot surveille alors **toute une large sélection** (sans que tu tapes
   quoi que ce soit) et se **concentre tout seul sur les 8 paires les plus
   volatiles**, en se **réajustant automatiquement toutes les 15 minutes**. Le
   mode « Je choisis mes paires » reste dispo si tu préfères. (En config :
   `top_movers: 8`, `0` = surveiller toutes tes paires.)
